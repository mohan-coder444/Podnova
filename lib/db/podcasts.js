...
export async function createPodcast({ userId, topic, options = {} }) {
  ...
  const { data, error } = await db.database
    ...
    .insert([{
      ...
      topic,
      options,                              // pass as plain object — JSONB column
      status:     'processing',
  ...
  if (error) {
    console.error('[createPodcast] DB error:', JSON.stringify(error))
    throw new Error(`DB insert failed: ${error?.message || error?.code || JSON.stringify(error)}`)
  }
  if (!data?.[0]) throw new Error('DB insert returned no data')
  return data[0]
}
...
export async function updatePodcast(id, updates) {
  ...
  if (error) {
    console.error('[updatePodcast] DB error:', JSON.stringify(error))
    throw new Error(`DB update failed: ${error?.message || error?.code || JSON.stringify(error)}`)
  }
}
...
export async function getPodcast(id) {
  ...
  if (error) {
    console.error('[getPodcast] DB error:', JSON.stringify(error))
    throw new Error(`DB fetch failed: ${error?.message || JSON.stringify(error)}`)
  }
  return data
}
...
export async function listPodcasts(userId) {
  ...
  if (error) {
    console.error('[listPodcasts] DB error:', JSON.stringify(error))
    throw new Error(`DB list failed: ${error?.message || JSON.stringify(error)}`)
  }
  return data || []
}
